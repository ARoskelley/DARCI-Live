...
/// Subjective experience: mood, energy, focus, current concerns.
///
/// v4 addition: <see cref="ToEncoderInput"/> converts this state
/// plus a perception snapshot into the numerical vector that feeds
/// the Decision Network. The Mood → valence mapping lives here so
/// Brain remains decoupled from Core types.
/// </summary>
public class State
{
    ...
    public float Energy { get; private set; } = 0.7f;
    public float Focus { get; private set; } = 0.5f;
    public float Engagement { get; private set; } = 0.5f;
    ...
    // =========================================================
    // v4 ADDITION: Neural encoding bridge
    // =========================================================

    /// <summary>
    /// Convert this state object + a perception snapshot into the flat numerical
    /// struct that <see cref="IStateEncoder"/> expects.
    ///
    /// The Mood → mood_valence mapping is defined here (not in Brain) because
    /// only Core knows what each Mood value means emotionally.
    /// </summary>
    public EncoderInput ToEncoderInput(Perception perception, float memoryRelevance = 0f)
    {
        // Pick the top waiting message for message-context dimensions.
        var topMsg = perception.NewMessages.FirstOrDefault();

        // Map MessageIntent confidence scores onto their respective dimensions.
        // If no intent or wrong type, those dimensions stay 0.
        float intentConv = 0, intentReq = 0, intentResearch = 0, intentFeedback = 0;
        if (topMsg?.Intent != null)
        {
            var conf = topMsg.Intent.Confidence;
            switch (topMsg.Intent.Type)
            {
                case IntentType.Conversation or IntentType.Question or IntentType.StatusCheck:
                    intentConv = conf; break;
                case IntentType.Task or IntentType.GoalUpdate:
                    intentReq = conf; break;
                case IntentType.Research:
                    intentResearch = conf; break;
                case IntentType.Feedback:
                    intentFeedback = conf; break;
            }
        }

        return new EncoderInput
        {
            // Internal State
            Energy          = Energy,
            Focus           = Focus,
            Engagement      = Engagement,
            MoodValence     = MoodToValence(CurrentMood),
            MoodIntensity   = MoodIntensity,
            ...
            MessagesWaiting      = perception.NewMessages.Count,
            HasUrgentMessage     = perception.HasUrgentMessage,
            TimeSinceUserContact = perception.TimeSinceLastUserContact,
            TimeSinceLastAction  = perception.TimeSinceLastAction,
            ActiveGoalsCount     = perception.ActiveGoalsCount,
            GoalsWithPendingSteps = perception.GoalsWithPendingSteps,
            PendingMemories      = perception.PendingMemoriesToProcess,
            CompletedTasksWaiting = perception.CompletedTasks.Count,
            IsQuietHours         = perception.IsQuietHours,
            HasActiveGoal        = CurrentGoalId.HasValue,
            ConsecutiveRestCycles = ConsecutiveRestCycles,
            UserTrustLevel       = Traits.Trust,

            // Message Context
            TopMessageLength      = topMsg?.Content.Length ?? 0,
            TopMessageHasQuestion = topMsg?.Content.Contains('?') ?? false,
            TopMessageSentiment   = 0f,  // populated by LLM sentiment pass (Phase 2)
            IntentConversation    = intentConv,
    ...
    /// <summary>
    /// Maps a <see cref="Mood"/> value to a [-1, 1] valence score.
    /// Positive moods → positive values. Negative moods → negative.
    /// Neutral/reflective moods → near zero.
    /// </summary>
    public static float MoodToValence(Mood mood) => mood switch
    {
        Mood.Satisfied  =>  0.7f,
        Mood.Content    =>  0.5f,
        Mood.Playful    =>  0.6f,
        Mood.Curious    =>  0.4f,
        Mood.Alert      =>  0.2f,
        Mood.Reflective =>  0.1f,
        Mood.Calm       =>  0.0f,
        Mood.Frustrated => -0.5f,
        _               =>  0.0f
    };
    ...
    private bool ShouldPersist()
        => ConsecutiveRestCycles % 100 == 0 || MoodIntensity > 0.6f;
