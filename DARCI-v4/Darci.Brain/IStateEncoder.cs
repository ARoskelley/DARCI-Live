...
///
/// The output is always float[29] with values in [-1, 1] or [0, 1].
/// No English passes through this interface — only numbers.
/// </summary>
public interface IStateEncoder
{
    /// <summary>
    /// The number of dimensions in the state vector. Always 29.
    /// Matches the Decision Network's input layer size.
    /// </summary>
    int Dimensions { get; }

    /// <summary>
    /// Encode a perception snapshot into a 29-dimensional float array.
    /// </summary>
    /// <param name="input">Flat numeric summary of State + Perception from Core.</param>
    /// <returns>A new float[29] with all values clamped to their defined ranges.</returns>
